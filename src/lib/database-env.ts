...
export function getDatabaseUrlConfigError(): string | null {
  ...
  if (!directUrl) {
    return "DIRECT_URL est manquante ou vide. Prisma en a besoin avec Supabase : ajoutez DIRECT_URL=\"postgresql://...\" (souvent la même base sur le port 5432 sans pgbouncer ; voir la doc Supabase « Direct connection »).";
  }
  if (!POSTGRES_PROTOCOL.test(directUrl)) {
